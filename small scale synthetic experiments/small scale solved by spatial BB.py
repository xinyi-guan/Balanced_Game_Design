...
with open('small-scale-summary-results-spatial_BB.csv', mode='w', newline='') as file:
    ...
    for current_seed in np.flip(seed_vec):
        ...
        while (min(LB_list_modified) < float('inf')) & (current_elapsed_time <= 3600): #small-scale
        ...
        df.to_csv(f'small-scale-best_incumbent_game_design_randomseed_{current_seed}_spatial_BB.csv', index=False, header=False)
