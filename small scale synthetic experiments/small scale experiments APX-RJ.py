...
with open('small-scale-summary-results-10-instances_APX-RJ.csv', mode='w', newline='') as file:
    ...
    for current_seed in np.flip(seed_vec):
        ...
        # blank callback without node prunning        
            ...
            def __call__(self):
                ...
                if (count > 0):
                    ...
                    for i in range(current_num_branches):
                        ...
                        with lock:
                            modified_w_inv_times_normlaized_b_dict[child_node_seq_id] = normialized_LP_max_delta_pi #parent node's modified_w_inv_times_normlaized_b
                        print(f"child node sequence id {child_node_seq_id} has the parent node {self.get_node_ID()} with normialized_LP_max_delta_pi as {normialized_LP_max_delta_pi}, there are {count} non-binary-elements-in-a.")
       
                                

        # blank callback without customized node selection 
        class CustomNodeSelection(ModelCallbackMixin, cpx_cb.NodeCallback):
            ...
            def __call__(self):
                ...
                if len(node_depth_vec) >= 100:
                    ...
                    if (nodeselection_seed_vec[len(node_depth_vec)] > 2) and (node_depth_vec[-1] != node_depth_vec[-2]+1) and (node_depth_vec[-1] != node_depth_vec[-2]) and (selection_timing[-1]+1 != len(node_depth_vec)): 
        ...
        output_filename = f'small-scale_instance_randomseed_{current_seed}_N=11_K=12_APX-RJ.txt'
        ...
        df.to_csv(f'small-scale-best_incumbent_game_design_randomseed_{current_seed}_N=11_K=12_APX-RJ.csv', index=False, header=False)
